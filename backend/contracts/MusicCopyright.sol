...
contract MusicCopyrightRegistry is ERC721 {
    ...
    // Revenue tracking
    mapping(uint256 => uint256) public songEarnings; // songId -> total earnings for artist
    ...
    function payForAccess(uint256 _songId) external payable songExists(_songId) nonReentrant {
        ...
        s.accessCount += 1;
        songEarnings[_songId] += registrantAmount; // Track earnings
    ...
    function getSongEarnings(uint256 _songId) external view songExists(_songId) returns (uint256) {
        return songEarnings[_songId];
    }
    
    /// @notice Get earnings for multiple songs owned by an artist
    function getArtistEarnings(address _artist) external view returns (
        uint256[] memory songIds,
        uint256[] memory earnings,
        uint256 totalEarnings
    ) {
        uint256[] memory artistSongs = songsByRegistrant[_artist];
        uint256 count = artistSongs.length;
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            songIds[i] = songId;
            earnings[i] = songEarnings[songId];
            totalEarnings += songEarnings[songId];
        }
        
        return (songIds, earnings, totalEarnings);
    }
}